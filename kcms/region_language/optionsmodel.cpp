    ...
    , m_settings(parent->settings())
{
    ...
    connect(m_settings, &RegionAndLangSettings::numericChanged, this, [this] {
        QLocale locale;
        if (m_settings->numeric() == m_settings->defaultNumericValue() && !m_settings->lang().isEmpty()) {
            locale = QLocale(m_settings->lang());
        } else {
            locale = QLocale(m_settings->numeric());
        };
    ...
    connect(m_settings, &RegionAndLangSettings::timeChanged, this, [this] {
        QLocale locale;
        if (m_settings->time() == m_settings->defaultTimeValue() && !m_settings->lang().isEmpty()) {
            locale = QLocale(m_settings->lang());
        } else {
            locale = QLocale(m_settings->time());
        };
    ...
    connect(m_settings, &RegionAndLangSettings::monetaryChanged, this, [this] {
        QLocale locale;
        if (m_settings->monetary() == m_settings->defaultMonetaryValue() && !m_settings->lang().isEmpty()) {
            locale = QLocale(m_settings->lang());
        } else {
            locale = QLocale(m_settings->monetary());
        };
    ...
    connect(m_settings, &RegionAndLangSettings::measurementChanged, this, [this] {
        QLocale locale;
        if (m_settings->measurement() == m_settings->defaultMeasurementValue() && !m_settings->lang().isEmpty()) {
            locale = QLocale(m_settings->lang());
        } else {
            locale = QLocale(m_settings->measurement());
        };
